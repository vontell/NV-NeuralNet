###############################################################################
#   
#   Neural Net Model and Infrastructure for NV Center Classification
#
#   This code can be used to train a model on spectrum scans for Nitrogen
#   Vacancy Centers. The model can then be used to classify whether a spectra
#   scan is characteristic of a Nitrogen Vacancy Center in diamond
#
#   Neural network designed and written by Eric Wadkins (http://ericwadkins.com)
#   Infrastructure and testing written by  Aaron Vontell (http://avontell.com)
#   Initial idea from Michael Walsh
#   
#   Started on January 17th 2017, finished on January 25th, 2017
#
#   MOST RECENT STATISTICS: 92.11% accuracy with 190 training samples
#                           and 38 validation samples
#
###############################################################################
...
def model(learning_rate=learning_rate, dropout=dropout):
    '''
    Constructs the model for the neural network. Consists of a convolutional
    layer, with a rectified linear activation function. Includes normalization
    and dropout, as well as a softmax loss function.
    '''
...
# LOADING, TRAINING, AND SAVING METHODS #######################################
...
parser.add_argument("--train", 
                    help="the filename (without .mat extension) to train on")
...
parser.add_argument('--overwrite', dest='overwrite', action='store_true',
                    help="include with --train to overwrite existing saved models")
parser.add_argument('--graph', dest='graph', action='store_true',
                    help="include with --train to display relvant NN graphs")
parser.set_defaults(omit=False, overwrite=False, graph=False)

args = parser.parse_args()
if args.train:
    train_net(args.train, args.overwrite, args.graph)
if args.classify:
